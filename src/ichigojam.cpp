...
extern uint64_t ICHIGOJAM_FONT [];

std15::std15(float screenW, float screenH, int buffW, int buffH)
  : screenW(screenW)
  , screenH(screenH)
  , buffW(buffW)
  , buffH(buffH)
...
void std15::setChar(int x, int y, char c) {
  buff [y*buffW+x] = c;
}
  ...
  for (int cy = 0; cy < CHAR_H; cy++) {
    ...
    for (int cx = 0; cx < CHAR_W; cx++) {
      if (((line >> (CHAR_W-cx-1)) & 0x1) == 0x1) {
        ofSetColor(0xff);
        ofDrawRectangle((x*CHAR_W+cx)*dotW, (y*CHAR_H+cy)*dotH, dotW, dotH);
      }
    }
  }
}
